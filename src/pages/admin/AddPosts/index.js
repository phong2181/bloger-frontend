...
import {
  ...
  FaUndo, FaRedo, FaHighlighter, FaCloudDownloadAlt
} from "react-icons/fa";
import "./style.scss";
...
const AddPost = () => {
  ...
  const addPostMutation = useGetAddNewPostAD();
  const { data: authors, isLoading: isAuthLoading } = useGetUsersAD();

  // YÊU CẦU 1: Lọc danh sách tác giả chỉ lấy Admin và Staff
  ...
  const handleFiles = (files) => {
    if (files && files.length > 0) {
      setPost((prev) => ({ ...prev, avatar_post: files[0] }));
    }
  };

  const handleSavePost = async (status) => {
    // YÊU CẦU 2: Kiểm tra dữ liệu các thẻ Input và Textarea trống
    ...
    if (!post.description || !post.description.trim()) {
      toast.error("Vui lòng nhập mô tả ngắn cho bài viết!");
      return;
    }

    if (!editor || editor.getText().trim() === "") {
      toast.error("Vui lòng gõ nội dung chi tiết bài viết!");
      return;
    }

    // Kiểm tra nâng cao hơn khi người dùng bấm nút Xuất bản (published)
    if (status === "published") {
      if (!post.category_id) {
        toast.error("Vui lòng chọn danh mục bài viết!");
        return;
      }
      if (post.authorType === "real" && !post.author) {
    ...
    const loadingMessage = status === "draft" ? "Đang lưu bản nháp..." : "Đang xuất bản bài viết...";
    ...
    await toast.promise(
      addPostMutation.mutateAsync(formData),
      {
        ...
        success: () => {
          ...
          return status === "draft" ? "Đã lưu bản nháp thành công! 📝" : "Xuất bản bài viết thành công! 🎉";
        },
        error: (err) => {
          console.error("Lỗi:", err.response?.data?.errors);
      ...
      },
      {
        ...
        success: {
          duration: 4000,
          iconTheme: { primary: "#4a6cf7", secondary: "#fff" },
        },
        error: { duration: 4000 },
      }
    );
      ...
      <div className="page-header">
        <div className="header-content">
          ...
          <p className="page-subtitle">
            Viết, chỉnh sửa và quản lý nội dung lưu nháp hoặc xuất bản
          </p>
        </div>
      </div>

      <form onSubmit={(e) => e.preventDefault()} className="add-post-form">
        <div className="form-wrapper">
          ...
          <div className="main-content">
            ...
            <section className="form-section">
              <div className="section-header">
                <h2>Tiêu đề bài viết <span style={{ color: "red" }}>*</span></h2>
              </div>
            ...
            <section className="form-section">
              <div className="section-header">
                <h2>Mô tả ngắn <span style={{ color: "red" }}>*</span></h2>
            ...
            <section className="form-section">
              <div className="section-header">
                <h2>Nội dung chi tiết <span style={{ color: "red" }}>*</span></h2>
          ...
          <aside className="addpost-sidebar">
            ...
            <section className="sidebar-card">
              ...
              <div className="card-body">
                <ImageUploader value={post.avatar_post} onFiles={handleFiles} />
              </div>
            </section>
...
<div className="action-buttons-group">
              ...
              >
                ...
                {addPostMutation.isPending ? "Đang xử lý..." : "Lưu bản nháp"}
              </button>
              ...
              >
                ...
                {addPostMutation.isPending ? "Đang xử lý..." : "Xuất bản bài viết"}
              </button>
            </div>
